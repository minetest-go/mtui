...
func Setup(a *app.App) error {
	...
	r.HandleFunc("/api/mail/list", api.Feature(types.PRIV_MAIL, api.Secure(api.GetMails))).Methods(http.MethodGet)
	r.HandleFunc("/api/mail/{sender}/{time}", api.Feature(types.PRIV_MAIL, api.Secure(api.DeleteMail))).Methods(http.MethodDelete)
	r.HandleFunc("/api/mail/{sender}/{time}/read", api.Feature(types.PRIV_MAIL, api.Secure(api.MarkRead))).Methods(http.MethodPost)
	...
	r.HandleFunc("/api/mail/send/{recipient}", api.Feature(types.PRIV_MAIL, api.Secure(api.SendMail))).Methods(http.MethodPost)
