...
CREATE TABLE player_metadata (player VARCHAR(50) NOT NULL,metadata VARCHAR(256) NOT NULL,value TEXT,PRIMARY KEY(player, metadata),FOREIGN KEY (player) REFERENCES player (name) ON DELETE CASCADE );
CREATE TABLE player_inventories (layer VARCHAR(50) NOT NULL,inv_id INT NOT NULL,inv_width INT NOT NULL,inv_name TEXT NOT NULL DEFAULT '',inv_size INT NOT NULL,PRIMARY KEY(player, inv_id),   FOREIGN KEY (player) REFERENCES player (name) ON DELETE CASCADE );
CREATE TABLE player_inventory_items (   player VARCHAR(50) NOT NULL,inv_id INT NOT NULL,slot_id INT NOT NULL,item TEXT NOT NULL DEFAULT '',PRIMARY KEY(player, inv_id, slot_id),   FOREIGN KEY (player) REFERENCES player (name) ON DELETE CASCADE );
`
